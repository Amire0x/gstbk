...
impl PolyComm {
    ...
    pub fn setup(d_max: &BigInt) -> PP {
        ...
        let p = Scalar::<Bls12_381_1>::group_order();
        let q = p.pow(bound);
        PP {
            disc,
            g,
    ...
    pub fn commit(pp: &PP, coef_vec: &[Scalar<Bls12_381_1>]) -> (PolyComm, BigInt) {
        unsafe { pari_init(100000000000, 2) };
    ...
    pub fn open(self, pp: &PP, coef_vec: &[Scalar<Bls12_381_1>]) -> Result<(), ErrorReason> {
        unsafe { pari_init(100000000000, 2) };
        let two = BigInt::from(2);
        let p_minus1_half = (&pp.p - &BigInt::one()).div_floor(&two);
        let coef_vec_int = (0..coef_vec.len())
    ...
    pub fn encode(p: &BigInt, q: &BigInt, coef_vec: &[Scalar<Bls12_381_1>]) -> BigInt {
    ...
    pub fn decode(p: &BigInt, q: &BigInt, y: &BigInt) -> Vec<Scalar<Bls12_381_1>> {
        let one = BigInt::one();
        ...
        let mut coef_vec: Vec<Scalar<Bls12_381_1>> = Vec::new();
        ...
        coef_vec.push(Scalar::<Bls12_381_1>::from(&f_0));
        if d == one {
            return coef_vec;
        }
        while d > one {
            ...
            coef_vec.push(Scalar::<Bls12_381_1>::from(&f_k));
            q_k = q_k_plus_1;
            d -= &one;
        }
        coef_vec
    ...
    pub fn eval_prove(
        &self,
        pp: &PP,
        z: &Scalar<Bls12_381_1>,
        y: &Scalar<Bls12_381_1>,
        coef_vec: &[Scalar<Bls12_381_1>],
    ) -> NiEvalProof {
        unsafe { pari_init(100000000000, 2) };
    ...
    pub fn eval_bounded_prove(
        ...
        let y_prime_fe = Scalar::<Bls12_381_1>::from(&y_prime);
...
impl NiEvalProof {
    pub fn eval_verify(
        ...
        if self.d == 0 {
            ...
            if y != &Scalar::<Bls12_381_1>::from(&self.f_const.mod_floor(&pp.p)) {
                flag = false;
            }
...
mod tests {
    ...
    fn test_commit_open() {
        ...
        for _ in 1..2 {
            ...
            while i < 10 {
                ...
                coef_vec.push(Scalar::<Bls12_381_1>::random());
                i += 1;
            }
    ...
    fn test_encode_decode_toy_example() {
        ...
        coef_vec.push(Scalar::<Bls12_381_1>::from(&BigInt::from(2)));
        coef_vec.push(Scalar::<Bls12_381_1>::from(&BigInt::from(3)));
        coef_vec.push(Scalar::<Bls12_381_1>::from(&BigInt::from(4)));
        coef_vec.push(Scalar::<Bls12_381_1>::from(&BigInt::from(1)));
    ...
    fn test_eval() {
        ...
        for _ in 0..6 {
            let (c, _f_q) = PolyComm::commit(&pp, &coef_vec);
            // generate y,z
            let z = Scalar::<Bls12_381_1>::random();

            let mut coef_vec_rev = coef_vec.iter().rev();
            let head = coef_vec_rev.next().unwrap();
            let tail = coef_vec_rev;

            let y: Scalar<Bls12_381_1> = tail.fold(head.clone(), |acc, x| x + &(acc * &z));
            //create proof:
    ...
    fn test_eval_wrong_eval() {
        ...
        let bias = Scalar::<Bls12_381_1>::from(&BigInt::from(2));
        let y = y + bias;
        //create proof:
        let proof = c.eval_prove(&pp, &z, &y, &coef_vec[..]);
        let result = proof.eval_verify(c.c, &pp, &z, &y);
